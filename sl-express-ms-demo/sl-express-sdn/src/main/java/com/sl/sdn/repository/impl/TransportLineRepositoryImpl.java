...
public class TransportLineRepositoryImpl implements TransportLineRepository {
    ...
    public TransportLineNodeDTO findShortestPath(AgencyEntity start, AgencyEntity end) {
        ...
        //构造查询语句
        String cypherQuery = CharSequenceUtil.format("MATCH path = shortestPath((start:{}) -[*..10]-> (end:{}))\n" +
                "WHERE start.bid = $startId AND end.bid = $endId \n" +
                "RETURN path", type, type);
        Optional<TransportLineNodeDTO> optional = neo4jClient
                .query(cypherQuery)
                ...
                .mappedBy((key, value) -> {
                    ...
                    path.nodes().forEach(node -> {
                        ...
                        InternalPoint2D location = MapUtil.get(map, "location", InternalPoint2D.class);
                        organDTO.setLatitude(location.x());
                        organDTO.setLongitude(location.y());
