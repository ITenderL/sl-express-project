...
public class CourierMQListener {
    ...
    public void listenCourierTaskMsg(String msg) {
        ...
        // 幂等处理，判断订单对应的取派件任务是否存在，判断条件：点个单号+任务状态（NEW）+任务类型
        List<PickupDispatchTaskEntity> list = pickupDispatchTaskService.findByOrderId(courierTaskMsg.getOrderId(), PickupDispatchTaskType.codeOf(courierTaskMsg.getTaskType()));
        ...
        if (ObjectUtil.isNull(result)) {
            // 保存任务失败
            throw new SLException(CharSequenceUtil.format("快递员任务保存失败 >>> msg = {}", msg));
        }
    }

    /**
     * 监听快递员微服务，快递员取件成功，订单转运单
     *
     * @param msg 消息
     */
    @RabbitListener(bindings = @QueueBinding(
